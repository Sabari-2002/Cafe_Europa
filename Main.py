...
class CafeApp:
    ...
    def show_login_page(self):
        ...
        # Background image
        bg_image = Image.open("Background_1.jpg").resize((1280, 800), Image.Resampling.LANCZOS)
    ...
    def show_menu_page(self):
        ...
        # Background
        bg_image = Image.open("Background_3.jpg").resize((1280, 800), Image.Resampling.LANCZOS)
        self.bg_photo = ImageTk.PhotoImage(bg_image)
        Label(self.root, image=self.bg_photo).place(x=0, y=0, relwidth=1, relheight=1)
        ...
        for i, (item, price) in enumerate(self.menu_items, start=1):
            ...
            Label(left_frame, text=f"₹{price}", font=("Arial",14), bg="#4A403C", fg="white").grid(
    ...
    def update_order_display(self):
        ...
        for i, (item, data) in enumerate(self.cart.items()):
            ...
            Label(self.order_display_frame, text=f"₹{data['total']}", font=("Arial",14), fg="white",
        ...
        if self.cart:
            ...
            Label(self.order_display_frame, text=f"Total: ₹{total_amount}", font=("Arial",16,"bold"), fg="yellow", bg="#4A403C").grid(
    ...
    def place_order(self):
        ...
        try:
            with open(filename, "w", encoding="utf-8") as file:
                ...
                file.write("-"*40 + "\n")
                for item, data in self.cart.items():
                    file.write(f"{item} | Qty: {data['qty']} | ₹{data['total']}\n")
                file.write("-"*40 + "\n")
                file.write(f"Subtotal: ₹{total}\n")
                file.write(f"GST (18%): ₹{gst_amount}\n")
